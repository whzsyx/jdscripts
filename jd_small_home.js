...
!(async () => {
  ...
  $.inviteCodes = await getAuthorShareCode('https://raw.githubusercontent.com/whzsyx/updateTeam/main/shareCodes/jd_updateSmallHomeInviteCode.json')
  if (!$.inviteCodes) {
    $.http.get({url: 'https://raw.githubusercontent.com/whzsyx/updateTeam/main/shareCodes/jd_updateSmallHomeInviteCode.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.inviteCodes = await getAuthorShareCode('https://raw.githubusercontent.com/whzsyx/updateTeam/main/shareCodes/jd_updateSmallHomeInviteCode.json')
  }
  for (let i = 0; i < cookiesArr.length; i++) {
